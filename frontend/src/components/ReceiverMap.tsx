...
export function ReceiverMap() {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-slate-900 bg-slate-900/20 backdrop-blur-xl p-6 shadow-xl shadow-slate-950/40">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white tracking-tight">Nearby Rescue Listings</h2>
              <p className="mt-1 text-xs sm:text-sm text-slate-400">Markers update in real time when new listings appear.</p>
            </div>
          </div>
          <div className="self-start sm:self-auto rounded-full bg-slate-900 border border-slate-800 px-4 py-1.5 text-xs font-semibold text-slate-200 flex items-center gap-2">
        ...
        <div className="mt-6 space-y-4">
          <div className="flex flex-col md:flex-row gap-3">
            <div className="relative flex-1 group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500 group-focus-within:text-cyan-400 transition-colors duration-200" />
              <input
                type="text"
                value={searchText}
                onChange={(event) => setSearchText(event.target.value)}
                className="w-full rounded-2xl border border-slate-800 bg-slate-950/50 pl-11 pr-4 py-3.5 text-sm text-slate-100 outline-none transition focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 placeholder:text-slate-650"
            ...
            <button
              ...
              className="rounded-2xl bg-cyan-500 px-6 py-3.5 text-sm font-bold tracking-wider uppercase text-slate-950 hover:bg-cyan-400 transition-all duration-200 shadow-lg shadow-cyan-500/10 shrink-0"
            >
              Search
            </button>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pt-2 border-t border-slate-900">
                ...
                <button
                  ...
                  className={`rounded-xl px-4 py-2 text-xs font-bold uppercase tracking-wider transition-all duration-200 ${
                    filterType === typeOption
                      ? "bg-cyan-500 text-slate-950 shadow-md shadow-cyan-500/5"
                      : "bg-slate-900 border border-slate-850 text-slate-400 hover:bg-slate-850 hover:text-slate-200"
                  }`}
                >
                  {typeOption}
                </button>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                disabled={locating}
                className="inline-flex items-center gap-1.5 rounded-xl bg-slate-950 hover:bg-slate-900 border border-slate-850 px-3.5 py-2 text-xs font-semibold text-slate-200 transition-colors duration-200 disabled:opacity-50"
              >
                {locating ? <Loader2 className="h-3.5 w-3.5 animate-spin text-cyan-400" /> : <Navigation className="h-3.5 w-3.5 text-cyan-400" />}
                <span>{locating ? "Locating..." : "Use device location"}</span>
              </button>
              <button
                type="button"
                onClick={handleUseAccountLocation}
                className="inline-flex items-center gap-1.5 rounded-xl bg-slate-950 hover:bg-slate-900 border border-slate-850 px-3.5 py-2 text-xs font-semibold text-slate-200 transition-colors duration-200"
              >
            ...
            <div className="flex items-center gap-3 bg-slate-900/50 border border-slate-900 rounded-xl px-3 py-1">
              ...
              <select
                ...
                className="bg-transparent text-xs font-semibold text-cyan-450 outline-none cursor-pointer py-1 pr-2 [color-scheme:dark]"
              >
          ...
          {message && (
            <div className="rounded-2xl border border-emerald-500/15 bg-emerald-500/5 px-4 py-3 text-xs text-emerald-300 flex items-center gap-2">
          ...
          {placeLoading ? (
            <div className="rounded-2xl border border-slate-850 bg-slate-950/20 px-4 py-8 text-center text-xs text-slate-500 flex flex-col items-center justify-center gap-2">
                  ...
                  <button
                    ...
                    className={`rounded-2xl border p-4 text-left transition-all duration-200 flex items-start gap-3.5 ${
                      isSelected
                        ...
                        : "border-slate-850 bg-slate-950/40 hover:border-slate-800 hover:bg-slate-900/60"
                    }`}
                  >
                    <div className={`rounded-xl p-2.5 transition-colors duration-200 shrink-0 ${
                      isSelected ? "bg-cyan-500 text-slate-950" : "bg-slate-950 border border-slate-850 text-slate-400"
                    }`}>
                      <PlaceIcon className="h-4.5 w-4.5" />
                    </div>
                    <div className="min-w-0">
                      <p className="font-bold text-white leading-tight truncate">{place.name}</p>
                      <p className="text-xs text-cyan-400 capitalize font-semibold mt-1">{place.category}</p>
                      {place.address && <p className="text-xs text-slate-400 mt-0.5 truncate">{place.address}</p>}
                    </div>
                  </button>
                );
              })}
        ...
        {error && (
          <div className="mt-4 rounded-2xl border border-rose-500/15 bg-rose-500/5 px-4 py-3 text-xs text-rose-300 flex items-center gap-2">
      ...
      <div className="h-[520px] rounded-3xl border border-slate-900 bg-slate-900/10 shadow-xl overflow-hidden relative">
        <MapContainer
          key={mapKey}
          center={center}
          zoom={13}
